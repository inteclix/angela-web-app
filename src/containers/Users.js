  ...
  render() {
    ...
    return (
      <div style={{flex: 1}}>
        ...
        {this.state.users.length === 0 && <div style={{
        flex: 1,
        justifyContent: "center",
        alignItems: "center"
      }}>No users found</div>}
        {this.state.users.length !==0 && 
          ...
          {this.state.users.map((user) => (
            <div style={{ alignItems: "center", padding: 5, flexDirection: "row", borderBottomWidth: 2, borderBottomColor: "lightgray", borderBottomStyle: "solid" }}>
              ...
              <IoMdTrash onClick={() => {
                const quetion = window.confirm("Are you sure to delete this user: " + user.username);
                quetion && WebServices.deleteUser(user.id).then(res => {
                  alert('User deleted success');
                  this.init();
                });
              }} color="red" size={32} />
              <IoIosEye onClick={() => this.props.history.push('/users/edit/' + user.id)} color="#3F51B5" style={{ marginLeft: 10 }} size={32} />
            </div>
          ))}
        </div>
        }
        <Fabs>
          <Fab Icon={IoMdAdd} onClick={() => this.props.history.push('/users/add')} />
        </Fabs>
      </div>
    )
  }
      ...
      <div>
        ...
        <div>
          ...
          <Label label="Is active">
            <input type="checkbox" onChange={(e) => { this.setState({ is_active: e.target.value }) }} value={this.state.is_active} placeholder="is_active" />
          </Label>
